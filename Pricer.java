...
//		PrintStream writer = new PrintStream(System.out);
		...
		tsize.close();
		Scanner keyboard = new Scanner(new BufferedReader(new FileReader("/Users/adamalloy/Downloads/pricer.in"))); // need to remove so not hard coded
		...
		public static void processLine(String currentLine, int[] BidsAsks, boolean[] NA, double[] last, Pricer pricer, Entry current) throws IOException{
			...
			if(instruction.equals("A")){
				...
				if(current.sd.equals("B")){
					...
					last[0] = PrintOrder(current, BidsAsks[0], false, last[0]);
					if(BidsAsks[0] >= targetsize)
						NA[0] = true;
				}
				if(current.sd.equals("S")){
					BidsAsks[1] += current.sz;
					last[1] = PrintOrder(current, BidsAsks[1], false, last[1]);
					if(BidsAsks[1] >= targetsize)
						NA[1] = true;
				}
			}
			if(instruction.equals("R")){
				current = ReduceOrder(timestamp, parse.next(), Integer.parseInt(parse.next()), BidsAsks);
				if(current.sd.equals("B")){
					last[0] = PrintOrder(current, BidsAsks[0], NA[0], last[0]);
					if(BidsAsks[0] < targetsize)
						NA[0] = false;
				}
				if(current.sd.equals("S")){
					last[1] = PrintOrder(current, BidsAsks[1], NA[1], last[1]);
					if(BidsAsks[1] < targetsize)
						NA[1] = false;
				}
			}
...
//		writer.close();
	...
	private static Pricer.Entry ReduceOrder(int timestamp, String O_id, int size, int[] BidsAsks) {
...
//	private static void write
	...
	private static double PrintOrder(Entry current, int num, boolean remove, double last) throws IOException  {
		// TODO Auto-generated method stub
		//testSet();
		if(remove && num < targetsize){
			System.out.println(current.time + " " + (current.sd.equals("S") ? "B" : "S") + " NA");
			return 0.0;
		}
		Iterator<Entry> iter;
		if (num >= targetsize){
