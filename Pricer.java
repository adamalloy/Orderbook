...
public class Pricer {
	...
	//class which organizes input data
	public class Entry implements Comparable<Entry>{
		...
		public int compareTo(Entry other) {
			System.out.println("time: " + this.time + " order_id: " + this.oid + " side: " + this.sd + " price: " + this.p + " size: " + this.sz);
			System.out.println("time: " + other.time + " order_id: " + other.oid + " side: " + other.sd + " price: " + other.p + " size: " + other.sz);
	...
	public static void main(String args[]){
		...
		try {
		//hard coded to read from disk, bad style  keyboard = new Scanner(System.in) would be better
		...
		Entry current = null;
		//arrays to hold data about bids and asks so they aren't global
		int[] BidsAsks = {0,0};
...
//prints adds
	...
	private static Pricer.Entry ReduceOrder(int timestamp, String O_id, int size, int[] BidsAsks, boolean[] NA) {
		...
		if((tmp.sz-size) <= 0){
			System.out.println(tmp.oid + " " + O_id);
		...
		if(timestamp == 29070543){
			System.out.println("29070543");
			//testSet();
		}
	...
	//a method that prints the orderbook
